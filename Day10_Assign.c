...
int main() {
    ...
    if (ret == 0) {
        ...
        msg_t m1;
        m1.type = 1;
        printf("sender: Enter 2 no.s for the addition: ");
        ...
        printf("sender: Received addition from the parent is %d\n", m1.sum);
		printf("child completed......\n");
        exit(0);
    } else if (ret > 0) {
        // Parent process: receive the message and print it
        msg_t m2;
        printf("receiver: waiting for the No.s...\n");
        ret = msgrcv(mqid, &m2, sizeof(m2) - sizeof(long), 1, 0);
        if (ret < 0) {
            perror("msgrcv() failed");
            exit(1);
        }
        printf("receiver: No.s received: %d %d\n", m2.n1, m2.n2);
