...
int main() {
	...
	//void *ptr, *ptr1;
	//void *src, *des;
	fd = open("/home/user/Downloads/data.txt", O_RDONLY);
	if(fd < 0) {
		perror("open() failed");
		_exit(1);
	}
	...
	void *src = mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
	if(src == (void*)-1) {
		perror("mmap() failed");
		close(fd);
		_exit(3);
	...
	fd1 = open("/home/user/Downloads/MMAP.txt", O_CREAT | O_RDWR | O_TRUNC, 0777);
      if(fd1 < 0) {
          perror("open() failed");
          _exit(1);
      }
	...
	void *des = mmap(NULL, st.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd1, 0);
      if(des == (void*)-1) {
          perror("mmap() failed");
          close(fd1);
          _exit(3);
